...
from states.state import MovieHighBudgetState
...
@bot.message_handler(state="*", commands=["high_budget_movie"])
def search_by_rating(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, MovieHighBudgetState.genre)
    with bot.retrieve_data(message.from_user.id) as data:
        data["movie_by_rating"] = {"user_id": user_id}


@bot.message_handler(state=MovieHighBudgetState.genre)
def get_rating(message: Message) -> None:
    if message.text.lower() != 'нет' and check_genre(message.text.lower()):
        ...
        bot.set_state(message.from_user.id, MovieHighBudgetState.limit)
    else:
        bot.send_message(message.from_user.id, "Такого жанра нет в базе или совершенна ошибка в вводе!"
                                               "\nПопробуйте ещё раз")


@bot.message_handler(state=MovieHighBudgetState.limit)
def get_limit_1(message: Message) -> None:
    global high_budget_info_list
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id) as data:
            data["movie_by_rating"]['limit'] = message.text

            result = high_budget_movie(limit=data["movie_by_rating"]['limit'],
                                       genre=data["movie_by_rating"]['genre'])

            high_budget_info_list = _process_movie_results(result, int(data["movie_low_budget"]['limit']))
            data["movie_by_rating"]['result'] = high_budget_info_list

        if high_budget_info_list:
...
@bot.callback_query_handler(func=lambda call: call.data.split('#')[0] == 'high_movie')
...
def send_movie_page(message, page=1):
    paginator = InlineKeyboardPaginator(
        len(high_budget_info_list),
        current_page=page,
        data_pattern='high_movie#{page}'
    )

    bot.send_message(
        message.chat.id,
        high_budget_info_list[page - 1],
    ...
    new_search = History(user_id=user_id,
                         result=high_budget_info_list[page - 1],
                         date=date)
    new_search.save()